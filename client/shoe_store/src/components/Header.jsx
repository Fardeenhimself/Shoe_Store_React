...
const Home = () => {
  const { log, usr, totalQty } = useContext(AuthContext)
  ...
  return (
    <div className="navbar bg-base-100 ">
      <div className="navbar-start ">
        ...
        <Link to={`/`} className="btn mr-2 text-3xl"><SiNike /></Link>
      </div>
      ...
      <div className="navbar-end">
        <Link to={`/cart`} className="btn mr-2"><LuShoppingCart /></Link>
        {
          totalQty > 0 ? <Link to={`/cart`}><div className="badge badge-success badge-lg transform -translate-x-7 text-white font-bold">{totalQty}</div></Link> : ''
        }

        {
          log.ack === true ? (
            <>
              <Link className="btn mr-2" onClick={logOut}>Log Out</Link>
